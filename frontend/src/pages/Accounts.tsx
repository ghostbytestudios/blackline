...
function GoalSection({ account }: { account: Account }) {
  ...
  if (editing) {
    return (
      <div className="mt-3 space-y-2 rounded-lg bg-ink-900/60 p-3">
        <input
          value={name}
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              inputMode="decimal"
              placeholder="10000"
              className="w-full rounded border border-ink-700 py-1.5 pl-5 pr-2 text-sm"
            />
          </div>
          ...
          <button
            ...
            className="rounded border border-ink-700 p-1.5 text-slate-400 hover:bg-ink-700"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
  ...
  return (
    <div className="mt-3">
      ...
      <div className="mt-1.5 flex items-center justify-between text-xs">
        <span className={reached ? "font-medium text-emerald-400" : "text-slate-400"}>
    ...
    <Card>
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="text-xs text-slate-500">{account.org_name ?? "—"}</div>
        </div>
        <select
          value={account.account_type}
          onChange={(e) => update.mutate({ id: account.id, type_override: e.target.value })}
          className="rounded border border-ink-700 bg-ink-800 px-2 py-1 text-xs font-medium text-slate-400"
        >
          {ROLES.map((r) => (
      ...
      <div
        className={`mt-4 font-mono text-2xl font-bold tnum ${
          account.balance_minor < 0 ? "text-red-500" : "text-slate-100"
        }`}
      >
        {formatMoney(account.balance_minor, account.currency)}
      </div>
      <div className="mt-1 text-xs text-slate-500">As of {formatDate(account.balance_date)}</div>
